...
Proof.
  intros x y. apply iff_reflect. rewrite beq_id_true_iff.
  reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  unfold t_update. intros. destruct (beq_idP x x0).
...
Proof.
  ...
  intros. destruct (beq_idP x1 x). destruct (beq_idP x2 x). 
 unfold not in H. rewrite <- e0 in e. symmetry in e.  apply H in e. inversion e.
 reflexivity.  destruct (beq_idP x2 x).  reflexivity.
 reflexivity.
Qed.
(** [] *)
